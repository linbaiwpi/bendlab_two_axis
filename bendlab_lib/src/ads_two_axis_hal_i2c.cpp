...
void ads_hal_delay(uint16_t delay_ms) { delay(delay_ms); }

void ads_hal_pin_int_enable(bool enable) {
  _ads_int_enabled = enable;

  if (enable) {
    attachInterrupt(digitalPinToInterrupt(ADS_INTERRUPT_PIN), ads_hal_interrupt,
                    FALLING);
  } else {
 ...
 */
uint8_t ads_hal_get_address(void) { return _address; }

/**
 * @brief Sets the i2c address that the hal layer is addressing *
 *				Used by device firmware update (dfu)
 */
void ads_hal_set_address(uint8_t address) { _address = address; }
